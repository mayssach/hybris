...
<c:set var="reviewAvgRating"  value="avgRating"/>
				...
				<c:if test="${not empty facetData.topValues}">
					<ul class="facet__list js-facet-list js-facet-top-values">
						<c:forEach items="${facetData.topValues}" var="facetValue">
							<li>
								<c:if test="${facetData.multiSelect and facetData.name ne reviewAvgRating}">
								...
								<c:if test="${facetData.multiSelect and facetData.name eq reviewAvgRating}">
									<form action="#" method="get">
										<!-- facetValue.query.query.value and searchPageData.freeTextSearch are html output encoded in the backend -->
										<input type="hidden" name="q" value="${facetValue.query.query.value}"/>
										<input type="hidden" name="text" value="${searchPageData.freeTextSearch}"/>
										<label>  <!--*********************************************** -->
											...
											<span class="facet__list__text">
												<div class="rating" >
													<input type="checkbox" ${facetValue.selected ? 'checked="checked"' : ''}  class="facet__list__checkbox js-facet-checkbox sr-only" >
													<div class="rating-stars pull-left js-ratingCalc ${fn:escapeXml(starsClass)}" data-rating='{"rating":"${fn:escapeXml(facetValue.name)}","total":5}' >
														<div class="greyStars">
